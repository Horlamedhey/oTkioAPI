...
router.post("/company/auction/addComment/:auctionId", async (req, res) => {
  try {
    ...
    if (req.body.replyingTo) {
      ...
      let comment = new Comment(_.pick(req.body, ["content", "replyingTo", "userId"]));
...
router.post("/company/auctions/addAttachment/:auctionId", upload.single("document"), async (req, res) => {
  try {
